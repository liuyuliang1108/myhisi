...
class Admin extends Common
{
    ...
    protected $config = [
        ...
        'expire' => 3600,                   //缓存时间
        'prefix' => 'user_menu_',            //缓存前缀
        'cache_tag' => 'menu',                 //缓存key
        ...
        if (!defined('ADMIN_ID')) {
            ...
            if (!$this->request->isAjax()) {
                ...
                if ($curMenu['id']) {
                    ...
                    if ($is_cache) {
                        //设置缓存name
                        $cache_key = $this->config['prefix']."breadCrumbs_".$curMenu['id'];
                        //获取缓存
                        $breadCrumbs = Cache::get($cache_key);
                        if (!$breadCrumbs) {//如未获取到缓存则，设置缓存
                            $breadCrumbs = MenuModel::getBreadCrumbs($curMenu['id']);
                            //设置缓存
                            Cache::tag($this->config['cache_tag'])->set($cache_key, $breadCrumbs, $this->config['expire']);
                        }
                    }
                    //判断是否开启缓存
                    if ($is_cache) {
                        //设置缓存name
                        $cache_key = $this->config['prefix']."menuParents_".$curMenu['id'];
                        //获取缓存
                        $menuParents = Cache::get($cache_key);
                        if (!$menuParents) {//如未获取到缓存则，设置缓存
                            $menuParents = current($breadCrumbs);
                            //设置缓存
                            Cache::tag($this->config['cache_tag'])->set($cache_key, $menuParents, $this->config['expire']);
                        }
                    }
                ...
                if ($is_cache) {
                    //设置缓存name
                    $cache_key = $this->config['prefix']."mainMenu";
                    //获取缓存
                    $mainMenu = Cache::get($cache_key);
                    if (!$mainMenu) {//如未获取到缓存则，设置缓存
                        $mainMenu = MenuModel::getMainMenu();
                        //设置缓存
                        Cache::tag($this->config['cache_tag'])->set($cache_key, $mainMenu, $this->config['expire']);
                    }
                }
                ...
                // 获取导航菜单
                $this->assign('hisiMenus', $mainMenu);
